...
export async function prepareJpegUpload(file: File): Promise<PreparedJpegUpload> {
  if (file.type !== "image/jpeg") throw new Error("Only JPEG files are supported.");
  ...
  try {
    ...
    return { display, height: decoded.height, thumb, width: decoded.width };
  } finally {
    decoded.dispose();
  }
}
